var FilterBlock = React.createClass({
        ...
        keys.some(
          k =>
            o[k]
              .toString()
              .toLowerCase()
              .indexOf(this.state.filtertextvalue) !== -1
        )
      );
  ...
  voteFilterText: function(EO) {
    console.log(
      "FilterBlock: текст в поле фильтра изменен - " + EO.target.value
    );
  ...
  sort: function(EO) {
    console.log(
      "FilterBlock: isCheckedCheckbox is " + this.state.isCheckedCheckbox
    );
    this.setState(
      { isCheckedCheckbox: EO.target.checked },
  ...
  render: function() {
    ...
    return React.DOM.div(
      ...
      React.DOM.input({
        type: "checkbox",
        onClick: this.sort,
        checked: this.state.isCheckedCheckbox
      }),
      React.DOM.input({
        type: "text",
        name: "filterText",
        onChange: this.voteFilterText,
